...
class RNN_point_process_all():
    ...
    def fit(self, train_time, test_time, in_size, atribute_0 = None, no_steps = 10, h = None, no_epoch = 100, log = 1, log_epoch = 1, method ="Euler"):
        ...
        for e in range(no_epoch):
            z_, integral_ = RNN_point_process_all.integral(self, train_time, in_size, no_steps = no_steps, h = h, method = method)
            train_loss = RNN_point_process_all.loss(z_, integral_)
            ...
            self.model.eval()
            z_test, integral_test = RNN_point_process_all.integral(self, test_time, in_size, no_steps=no_steps, h=h, method='Trapezoid')
            test_loss = RNN_point_process_all.loss(z_test, integral_test)
...
if __name__ == "__main__":
    ...
    # evaluation_df.loc[len(evaluation_df)] = ['RNN_euler', 'synthetic', loss_on_train.data.numpy()[0][0], None]
