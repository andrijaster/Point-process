    ...
    for model_definition in models_to_evaluate:
        for params in model_definition['learning_param_map']:
            ...
            while not model:
                ...
                model_name = f"generated_exp-{type(model).__name__}-{params['learning_rate']}-{l_generator}_0.3"
                ...
                if model:
                    ...
                    model_filepath = f"{project_dir}/models/dummy/{model_name}_0.3.torch"
                    pickle.dump(model, open(model_filepath, 'wb'))

    print(evaluation_df)

    evaluation_df.to_csv(f"{project_dir}/results/generated_exp_nn_{str(learning_param_map[0]['learning_rate'])}_0.3.csv",
                         index=False)
