        ...
        for params in model_definition['learning_param_map']:
            ...
            while not model:
                ...
                model = BaseTraining.fit(model, train_time, test_time, in_size, lr=params['learning_rate'],
                                         ...
                                         figpath=f"{project_dir}/img/geoloc/{model_name}.png")
                if model:
                    ...
                    model_filepath = f"{project_dir}/models/geoloc/{model_name}.torch"
                    pickle.dump(model, open(model_filepath, 'wb'))

    print(evaluation_df)
    evaluation_df.to_csv(f"{project_dir}/results/zh_main_station_240820_{str(learning_param_map[0]['learning_rate'])}_0.2.csv",
                         index=False)
