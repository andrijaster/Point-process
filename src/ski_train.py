...
if __name__ == "__main__":
        ...
        for params in model_definition['learning_param_map']:
            ...
            while not model:
                ...
                model = BaseTraining.fit(model, train_time, test_time, in_size, lr=params['learning_rate'],
                                         ...
                                         figpath=f"{project_dir}/img/ski/{model_name}.png")
                if model:
    ...
    print(evaluation_df)
    evaluation_df.to_csv(f"results/ski_kg_2018_scores_{str(learning_param_map[0]['learning_rate'])}_0.5.csv", index=False)
